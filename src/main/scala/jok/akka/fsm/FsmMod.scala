...
object FsmMod {
  ...
  /**
    * Send this to an [[akka.actor.FSM]] to request first the [[FsmMod.CurrentState]]
    * and then a series of [[FsmMod.Transition]] updates. Cancel the subscription
    * using [[FsmMod.UnsubscribeTransitionCallBack]].
    */
...
/**
  ...
  * </pre>
  */
trait FsmMod[S, D] extends Actor with Listeners with ActorLogging {

  import FsmMod._

  type State = FsmMod.State[S, D]
  type Event = FsmMod.Event[D]
  type StopEvent = FsmMod.StopEvent[S, D]
   ...
   */
  val Event: FsmMod.Event.type = FsmMod.Event
  val StopEvent: FsmMod.StopEvent.type = FsmMod.StopEvent
  ...
  /**
    * This case object is received in case of a state timeout.
    */
  val StateTimeout = FsmMod.StateTimeout
  ...
  final def startWith(stateName: S, stateData: D, timeout: Timeout = None): Unit =
    currentState = FsmMod.State(stateName, stateData, timeout)
  ...
  /**
    ...
    */
  final def goto(nextStateName: S): State = FsmMod.State(nextStateName, currentState.stateData)
  ...
  private[akka] def makeTransition(nextState: State): Unit = {
    if (!stateFunctions.contains(nextState.stateName)) {
      ...
      if (currentState.stateName != nextState.stateName || nextState.notifies) {
        ...
        exitFunc match {
          case Some(exitF) => currentState = currentState.copy(stateData = exitF(currentState.stateData))
        ...
        entryFunc match {
          case Some(entryF) => currentState = nextState.copy(stateData = entryF(nextState.stateData))
          case None => currentState = nextState
        }
  ...
  /**
    * By default [[FsmMod.Failure]] is logged at error level and other reason
...
/**
  ...
  * @since 1.2
  */
trait LoggingFsmMod[S, D] extends FsmMod[S, D] {
  this: Actor ⇒
